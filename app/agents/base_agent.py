...
class BaseAgent(BaseModel):
    ...
    _conversation_history = PrivateAttr(default=None)
    ...
    async def _get_all_mcp_functions(self):
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{self.mcp_url}/.well-known/mcp")
                if response.status_code == 200:
                    mcp_schema = response.json()
                    functions = mcp_schema.get("functions", [])
                    all_functions.extend(functions)
        except Exception as e:
            print(f"Error fetching functions from {self.mcp_service}: {str(e)}")
    ...
    async def _call_mcp_function(self, function_name: str, function_args: Dict[str, Any]):
        function_url = f"{self.mcp_url}/functions/{function_name}"
        try:
            async with httpx.AsyncClient() as client:
